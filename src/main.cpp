...
#if defined(NDEBUG)
# error "Tardcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x783bcda81031423b89ee154d42d13a15c205bbe96827dac883907c299ca68781");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Tardcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Tardcoin Signed Message:\n";
...
{
    // Tardcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
bool CTransaction::CheckTransaction(CValidationState &state) const
{
    ...
    if (IsCoinBase())
    {
        if (vin[0].scriptSig.size() < 2 || vin[0].scriptSig.size() > 300)
            return state.DoS(300, error("CTransaction::CheckTransaction() : coinbase script size"));
    }
    else
    {
        BOOST_FOREACH(const CTxIn& txin, vin)
...
{
    int64 nSubsidy = 69420 * COIN;

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 696969); // Tardcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Tardcoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Tardcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Tardcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Tardcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x30f3dd5cb2907253ed8a70d734d01daf45da68f57f6f5bf9f940fc224e7ac1a4");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "I'm overclocking my last braincell not infected by an extra chromosome to accomplish this right now, but my cell is generating to much heat and the cooling system is brand new and already clogged with pure mega retardation essence. The tard is about to fully take me over.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 69420 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04bbe057348c43f96c37f058457b32e07e4d6540cd888828ccfcc72df14cc2b67b629700fbc7e1d7cb7e112dbd25288d570af04cf50718db778595407969899ec0") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1625204969;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2086367002;

        if (fTestNet)
        {
            block.nTime    = 1625204985;
            block.nNonce   = 385525220;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xe6ee5180534f50e88c028613110ac8321062cab5bde4e290f21e0ad69764aa7e"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa9, 0xb0, 0xe9, 0xac }; // Tardcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TardcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TardcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TardcoinMiner(CWallet *pwallet)
{
    printf("TardcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tardcoin-miner");
    ...
    try { loop {
        ...
        printf("Running TardcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TardcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TardcoinMiner, pwallet));
}
